			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<li>
						...
						<?php echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity monospace">' . sprintf( '%s &times; %s', $cart_item['quantity'], $product_price ) . '</span>', $cart_item, $cart_item_key ); ?>
					</li>
					<?php
				}
			}
...
<?php if ( sizeof( WC()->cart->get_cart() ) > 0 ) : ?>
	...
	<p class="total monospace"><?php _e( 'Subtotal', 'woocommerce' ); ?>: <?php echo WC()->cart->get_cart_subtotal(); ?></p>
